...
mkdir -p /var/vcap/jobs/awslogs-trusty/conf.d

export CONFIG_FILE=/var/vcap/jobs/awslogs-trusty/conf.d/all-vcap-logs.conf
...
scan_for_logs() {
 ...
 find /var/vcap/sys/log -type f | while read -r I
  ...
  if cmp -s "${TMPCONF}" "${CONFIG_FILE}"; then
    ...
    /var/vcap/bosh/bin/monit restart awslogs-trusty
  fi
}
...
while ${INOTIFY_WAIT} -r -e create /var/vcap/sys/log @/var/vcap/sys/log/awslogs-trusty; do
